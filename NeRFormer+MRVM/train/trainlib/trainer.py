...
class Trainer:
    def __init__(self, net, train_dataset, test_dataset, args, conf, device=None):
        ...
        if args.warm_up_iter > 0:
            warm_up_func = lambda iter: (iter/args.warm_up_iter)*args.lr
            
            self.scheduler_warm_up = torch.optim.lr_scheduler.LambdaLR(self.optim, warm_up_func)
            self.scheduler_warm_up.base_lrs = [1.0]
        else:
            self.scheduler_warm_up = None
        ...
        if self.stage == 'fine_tune':
            path_to_load = os.path.join(self.args.path_to_load, 'net'+'_' +str(args.load_iter))
            if os.path.exists(path_to_load):
                net.load_state_dict(
                        torch.load(path_to_load, map_location=device)
                    )
                print('Fine tune stage: load checkpoints from', path_to_load)
            else:
                raise ValueError("attempt to load ckpt which path doesn't exist!")
        if args.resume:
            ...
            if os.path.exists(self.iter_state_path):
                ...
                self.start_epoch_id = torch.load(
                    self.iter_state_path, map_location=device
                )["epoch"]
    ...
    def set_arguments(self):
        if self.stage == 'pretrain':
            self.args.name = self.args.name + '_prt' 
        else:
            assert self.stage == 'fine_tune'
            self.args.name = self.args.name + '_ft' \
            +'_from_{}_'.format(self.args.load_iter) \
            + self.args.path_to_load.split('/')[-1]
            self.net.mask_ratio = 0.0
            self.args.lambda_recons = 0.0
            self.net.stage = 'fine_tune'

        os.makedirs(os.path.join(self.args.checkpoints_path, self.args.name), exist_ok=True)
        os.makedirs(os.path.join(self.args.visual_path, self.args.name), exist_ok=True)
    ...
    def start(self):
        ...
        for epoch in range(self.num_epochs):
            if epoch < self.start_epoch_id:
                continue
            if step_id > self.itera:
                print("{} stage ended successfully at iter_{}".format(self.stage, step_id))
                break
            ...
            for data in self.train_data_loader:
                self.net.train()
                losses = self.train_step(data, global_step=step_id, stage=self.stage)
                ...
                if step_id % self.save_interval == 0 :
                    ...
                    torch.save({"iter": step_id + 1, "epoch": epoch}, self.iter_state_path)
                    self.extra_save_state()
                
                if (self.stage == 'pretrain' and step_id % 50000 == 0) or \
                    (self.stage =='fine_tune' and step_id % 50000 == 0):
                    print("extra ckpt saving")
                    torch.save(
                            self.net.state_dict(), self.default_net_state_path + '_' +str(step_id)
                        )
                ...
                self.optim.step()
                if self.stage == 'pretrain' and step_id >= self.args.stage_two_begin:
                   self.net.updata_target()
                if step_id < self.args.warm_up_iter:
                    self.scheduler_warm_up.step()
