...
def extra_args(parser):
    ...
    parser.add_argument(
        '--load_fine_tune_iter', 
        type=int, default=1000000, help='choose from 50000, 100000, 150000, 200000',
    )
...
args.resume = True
conf["model"]["mask_ratio_on_a_ray"] = args.mask_ratio
conf["model"]["stage"] = args.stage
...
if args.path_to_load is not None:
    path_to_load = os.path.join(args.path_to_load, 'net'+'_' +str(args.load_fine_tune_iter))
...
dset = get_split_dataset(
    args.dataset_format, args.datadir, want_split=args.split, training=False, leinao=args.leinao
)
