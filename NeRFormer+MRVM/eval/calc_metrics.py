...
parser.add_argument(
    ...
    "-D",
    type=str,
    default="/data/yanggl/pixel_nerf_data/NMR_Dataset",
...
parser.add_argument(
    "--metadata",
    type=str,
    default="/data/yanggl/pixel_nerf_data/NMR_Dataset/metadata.yaml",
...
def run_map():
    ...
    def process_obj(path, rend_path):
        ...
        if use_exclude_lut:
            ...
            exclude_views = list(map(int, exclude_lut[lut_key]))
        else:
            exclude_views = []
        ...
        for im_name in ims:
            ...
            if osp.exists(im_rend_path) and im_name_id not in exclude_views:
                ...
                psnr_avg += psnr
                ssim_avg += ssim
                gts.append(torch.from_numpy(gt).permute(2, 0, 1) * 2.0 - 1.0)  # (H, W, C)->(C, H, W) && [0,1]->[-1,1]
                preds.append(torch.from_numpy(pred).permute(2, 0, 1) * 2.0 - 1.0)
                num_ims += 1
        gts = torch.stack(gts)  # (num_images, C, H, W)
        ...
        with torch.no_grad():
            for predi, gti in zip(preds_spl, gts_spl):
                lpips_i = lpips_vgg(predi.to(device=cuda), gti.to(device=cuda)) # (lpips_bs,1,1,1)
                lpips_all.append(lpips_i)
            lpips = torch.cat(lpips_all)  # (num_images, 1, 1, 1)
        lpips = lpips.mean().item()  #float num
        psnr_avg /= num_ims
        ssim_avg /= num_ims
