...
args, conf = util.args.parse_args(
    extra_args, default_conf="conf/resnet_fine_mv.conf", default_expname="shapenet", default_ray_batch_size=10000000
)
args.resume = True
args.name = args.name + '_' + str(args.load_iter)
if not args.multicat:
    args.name = args.name + '_vi_{}'.format(len(args.source.split(',')))
conf["model"]["mask_ratio_on_a_ray"] = 0.0
conf["model"]["stage"] = 'eval'
conf["model"]["coarse_pt_num"] = conf['renderer']['n_coarse']
conf["model"]["fine_pt_num"] = conf['renderer']['n_coarse'] + conf['renderer']['n_fine'] 
...
data_loader = torch.utils.data.DataLoader(
    dset, batch_size=1, shuffle=False, num_workers=args.num_worker, pin_memory=False
)

output_dir = os.path.join(args.output.strip(), args.name)
...
if has_output:
    ...
    if os.path.exists(finish_path):
        ...
        lines = [x for x in lines if len(x) == 5]   # when adding LPIPS, it should be 5
...
if args.path_to_load is not None:
    path_to_load = os.path.join(args.path_to_load, 'net'+'_' +str(args.load_iter))
    if os.path.exists(path_to_load):
        net.load_state_dict(
                torch.load(path_to_load, map_location=device)
            )
        print('Validation: load checkpoints from', path_to_load)
    else:
...
with torch.no_grad():
    for obj_idx, data in enumerate(data_loader):
        ...
        for rays in tqdm.tqdm(rays_spl):
            rgb, depth = render_par(rays[None])  # shape: (1, ray_bs, 8)
            rgb = rgb[0].cpu()
            depth = depth[0].cpu()
            all_rgb.append(rgb)
        ...
        if not args.no_compare_gt:
            ...
            for view_idx in range(n_gen_views):
                ...
                if args.write_compare:
                    image_source = images[src_view_mask] * 0.5 + 0.5 # (NS,3,H,W)
                    image_source = image_source.permute(0, 2, 3, 1).contiguous().numpy() #(NS,H,W,3)
            
                    for source_num in range(len(image_source)):
                        source_out_path  = os.path.join(obj_out_dir, "{:06}_source.png".format(source_num))
                        imageio.imwrite(source_out_path, (image_source[source_num]*255).astype(np.uint8))
            ...
            curr_lpips = lpips_vgg(torch.from_numpy(all_rgb).to(device).permute(0, 3, 1, 2) * 2.0 - 1.0,
                                  torch.from_numpy(rgb_gt_all).to(device).permute(0, 3, 1, 2) * 2.0 - 1.0).mean().item()
