...
class NeRFRenderer(torch.nn.Module):
    ...
    def composite(self, model, rays, z_samp, coarse=True, sb=0):
        ...
        with profiler.record_function("renderer_composite"):
            B, K = z_samp.shape  # B: SB*num_rays, K:num_points
            ...
            # (B, K, 3)
            points = rays[:, None, :3] + z_samp.unsqueeze(2) * rays[:, None, 3:6]  # points.shape: (SB*num_ray, num_points, 3)
            ...
            if sb > 0:
                points = points.reshape(
                    sb, -1, K, 3
                )  # (SB, num_ray, num_points, 3)
                eval_batch_size = self.eval_batch_size // sb
                eval_batch_dim = 1
            else:
            ...
            if use_viewdirs:
                dim1 = K
                viewdirs = rays[:, None, 3:6].expand(-1, dim1, -1)  # (sb*num_rays, K, 3)
                if sb > 0:
                    viewdirs = viewdirs.reshape(sb, -1, K, 3)  # (SB, num_rays, K, 3)
                else:
                ...
                for pnts, dirs in zip(split_points, split_viewdirs):
                    returned = model(pnts.reshape(sb, -1, 3), 
                                     coarse=coarse, 
                                     viewdirs=dirs.reshape(sb, -1, 3))
                    val_all.append(returned[0])
                    useful_feat.append(returned[1])
            else:
            ...
            viewdirs = None
            # (B*K, 4) OR (SB, B'*K, 4)
            if useful_feat[0] is not None:
                latent_use = torch.cat(useful_feat, dim=eval_batch_dim) # shape: (SB, num_rays, num_points, projection_size)
            else:
                latent_use = None

            out = torch.cat(val_all, dim=eval_batch_dim)  # shape: (SB, num_rays, num_points, 4)
            out = out.reshape(B, K, -1)  # (SB*num_rays, num_points, 4)
    ...
    def forward(
        ...
        with profiler.record_function("renderer_forward"):
            ...
            target_represent = coarse_composite[-1] # shape:(SB, num_rays, coarse_points, projection_size)
            outputs = DotMap(
            ...
            if self.using_fine:
                ...
                if self.n_fine_depth > 0:
                    all_samps.append(
                        self.sample_fine_depth(rays, coarse_composite[2].detach())
                    )  # (B, Kfd)
                z_combine = torch.cat(all_samps, dim=-1)  # (B, Kc + Kf)
                z_combine_sorted, argsort = torch.sort(z_combine, dim=-1)  # (SB*num_rays, fine_points)
                ...
                online_represent = fine_composite[-1] # shape: (SB, num_rays, fine_points, projection_size)
                if (online_represent is not None) and (target_represent is not None): # pretrain
                    coarse_pt_idx = argsort.argsort(dim=-1)[:, :self.n_coarse].reshape(superbatch_size, -1, self.n_coarse) #(SB, num_rays, coarse_points)
                    # (SB, num_rays, coarse_points, projection_size)
                    useful_online = torch.gather(online_represent, 2, coarse_pt_idx.unsqueeze(-1).expand(-1,-1,-1, *online_represent.shape[3:])) 
                    feat_diff = loss_fn(target_represent.detach(), useful_online) # (SB, num_rays, coarse_points)
                    outputs.feat_diff = feat_diff
            return outputs

    def _format_outputs(
        self, rendered_outputs, superbatch_size, want_weights=False,
    ):
        weights, rgb, depth, _ = rendered_outputs
    ...
    @classmethod
    def from_conf(cls, conf, white_bkgd=False, lindisp=False, eval_batch_size=256*4):
        return cls(
