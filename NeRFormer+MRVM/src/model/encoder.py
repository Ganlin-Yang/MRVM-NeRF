...
class SpatialEncoder(nn.Module):
    ...
    def index(self, uv, cam_z=None, image_size=(), z_bounds=None):
        ...
        with profiler.record_function("encoder_index"):
            ...
            rgbs = F.grid_sample(
                self.origin_image.to(uv.device),
                uv,
                align_corners=True,
                mode=self.index_interp,
                padding_mode=self.index_padding,
            )
            return torch.cat((rgbs, samples),dim=1)[...,0]  # (B, C, N)
    ...
    def visulize_attention_map(self, obj_out_dir):
        input_image = self.latent.mean(dim=1).unsqueeze(1)
        img_after_scale = F.interpolate(input_image, scale_factor=2.0)
        shape = img_after_scale.shape
        img_after_scale_ = img_after_scale.reshape(shape[0], -1)
        max, min = torch.max(img_after_scale_, dim=-1)[0], torch.min(img_after_scale_, dim=-1)[0]
        max, min = max.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1), min.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
        # normalize to 0~1
        img_after_norm = (img_after_scale-min)/(max-min)
        for i in range(len(img_after_norm)):
            plt.figure()
            plt.imshow(img_after_norm[i,0].cpu().detach().numpy())
            plt.colorbar()
            plt.savefig(obj_out_dir + '/attention_map_{}.png'.format(i))
            plt.show()
