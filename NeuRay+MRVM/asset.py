import os
import numpy as np
from globals import base_data_dir_lst
base_data_dir = base_data_dir_lst[0]

gso_scene_names, gso_scene_names_400= [], []
if os.path.exists (os.path.join(base_data_dir, 'google_scanned_objects')):
    for fn in os.listdir(os.path.join(base_data_dir, 'google_scanned_objects')):
        if os.path.isdir(os.path.join(base_data_dir, 'google_scanned_objects',fn)):
...
if os.path.exists(os.path.join(base_data_dir, 'dtu_train')) and os.path.exists(os.path.join(base_data_dir, 'dtu_test')):
    fns = os.listdir(os.path.join(base_data_dir, 'dtu_train'))
    fns = [fn for fn in fns if fn.startswith('scan')]
    test_scenes = os.listdir(os.path.join(base_data_dir, 'dtu_test'))

    test_scans = np.loadtxt('configs/dtu_test_scans.txt',dtype=str).tolist()
...
if os.path.exists(os.path.join(base_data_dir, 'real_iconic_noface')):
    fns = os.listdir(os.path.join(base_data_dir, 'real_iconic_noface'))
    real_iconic_scene_names_8 = [f'real_iconic/{fn}/8' for fn in fns]
    real_iconic_scene_names_4 = [f'real_iconic/{fn}/4' for fn in fns]

space_scene_names = []
if os.path.exists(os.path.join(base_data_dir, 'spaces_dataset')):
    fns = os.listdir(os.path.join(base_data_dir,'spaces_dataset/data/800'))
    space_scene_names = [f'space/{fn}' for fn in fns]

real_estate_scene_names = []
if os.path.exists(os.path.join(base_data_dir, 'real_estate_dataset')):
    fns = os.listdir(os.path.join(base_data_dir, 'real_estate_dataset/train/frames'))
...
LLFF_ROOT = os.path.join(base_data_dir, 'llff_colmap')

NERF_SYN_ROOT = os.path.join(base_data_dir, 'nerf_synthetic')