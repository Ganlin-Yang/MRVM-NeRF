...
class PSNR_SSIM(Loss):
    ...
    def __call__(self, data_pr, data_gt, step, **kwargs):
        ...
        ssim = compare_ssim(rgbs_gt,rgbs_pr,win_size=11,multichannel=True,data_range=255)
        outputs={
        ...
        def compute_psnr_prefix(suffix):
            if f'pixel_colors_{suffix}' in data_pr:
                ...
                ssim = compare_ssim(rgbs_gt,rgbs_other,win_size=11,multichannel=True,data_range=255)
...
class VisualizeImage(Loss):
    ...
    def __call__(self, data_pr, data_gt, step, **kwargs):
        ...
        Path(os.path.join(save_dir, f'vis_val/{model_name}')).mkdir(exist_ok=True, parents=True)
        if h<=64 and w<=64:
            imsave(os.path.join(save_dir, f'vis_val/{model_name}/step-{step}-index-{data_index}.png'), concat_images_list(*imgs))
        else:
            imsave(os.path.join(save_dir, f'vis_val/{model_name}/step-{step}-index-{data_index}.jpg'), concat_images_list(*imgs))
        return outputs
